...
def model_assumptions_speedup(colors={}, markers={}):
    yaml = ruamel.yaml.YAML()
    ...
    mass_keys.sort()
    mass_keys = sorted(mass_keys[:4], key=len) + sorted(mass_keys[4:], key=len)
    ...
    fig.set_figheight(8)
    bwid = 0.15
    xlabs = [m.split("-mass")[-1] for m in mass_keys[:4]]
    xlabs[0] = "-std"
    ...
    plt.savefig("figures/reaction-bar.pdf")
    plt.close()
